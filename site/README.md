...
The repository deliberately carries the complete accepted CON snapshot: 186 canonical records, 13 reference records, ten editorial sources, 71 declared content assets, the accepted presentation snapshot, and consumer-specific behavior tests.
...
`validate` and `build` depend on `projection-update`, which recreates ignored
pages, the record stream, and graph under `generated/projection/`.
Review the source metadata diff and the rendered build; do not commit or
...
The presentation framework, including its compiled stylesheet and images, is
stored as ordinary Git files. Building and viewing the site requires neither
DataLad nor an external large-file backend.
