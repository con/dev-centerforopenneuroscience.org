...
The repository deliberately carries the complete accepted CON snapshot: 186 canonical records, 13 reference records, ten editorial sources, 71 declared content assets, the committed generated projection, the accepted presentation snapshot, and the consumer-specific acceptance tests.
...
| `metadata/provenance/` | Source, selection, and human-review evidence. |
...
| `generated/` | Replaceable outputs and review ledgers; regenerate only through the declared commands. |
...
The updater protects site-owned and generated bytes and stops for review rather than silently reconciling them.

## Edit, validate, and preview

For a canonical metadata change:
...
`projection-update` refreshes the committed pages, record stream, graph, and digests under `generated/projection/`.
Review those generated changes together with the canonical YAML that caused them.
...
Run hydration first in a cold clone or before beginning the documented [warmed-cache offline acceptance procedure](../tests/offline/README.md).

This policy is distinct from the 13 inherited git-annex pointers once present in the accepted presentation framework.
Those 13 paths—including the compiled stylesheet—are already materialized in `site/framework/` as verified ordinary Git payloads.
Their source pointers, annex keys, payload digests, and materialization source are recorded in `generated/manifests/framework-import.json`.
A consumer needs neither DataLad nor git-annex to build or view the framework.
...
6. Run `projection-update`, `projection-verify`, `validate`, and the normal build and test review before committing the YAML and generated projection.

The browser has no repository token or metadata-service credential and makes no write request.
The local command rejects stale commits or source digests, unknown or duplicate PIDs, path escapes, identity or schema-type changes, invalid RDF, and a conflicting local change to a target canonical record.
Without `--write`, it does not modify canonical YAML.
...
2. review `generated/manifests/framework-update.json` for the old and new coordinates, protected-content hashes, conflicts, migrations, and test result;
3. require `site_owned.changed = []` for a framework-only update;
